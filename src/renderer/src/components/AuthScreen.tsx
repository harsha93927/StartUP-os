...
interface AuthScreenProps {
  onAuthSuccess: (data: { email: string }) => void
}
  ...
  const onSubmit = async (data: AuthFormData) => {
    ...
    try {
      const result = isRegistering
        ? await window.api.register({ email: data.email, password: data.password })
        : await window.api.login({ email: data.email, password: data.password })
        ...
        <div className="relative h-64 flex flex-col justify-center">
           <AnimatePresence mode="wait">
            <motion.p
              ...
              className="text-4xl font-medium leading-tight max-w-md tracking-tight text-foreground"
            >
        ...
        <div className="w-full max-w-[400px] space-y-8">
          <div className="space-y-2 text-center lg:text-left">
            ...
            <p className="text-muted-foreground text-sm font-medium">
              {isRegistering ? 'Start your journey with Startup OS' : 'Enter your credentials to access your workspace'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <input
                  {...register("password")}
                  className={`flex h-11 w-full rounded-xl border bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 \${errors.password ? 'border-red-500' : 'border-border hover:border-muted-foreground/50'} transition-all`}
                  id="password"
                  type="password"
                />
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase tracking-widest font-bold">
              <span className="bg-background px-3 text-muted-foreground">
                {isRegistering ? 'Already have an account?' : 'New to Startup OS?'}
              </span>
            </div>
          </div>
