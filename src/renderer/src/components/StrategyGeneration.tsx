...
export default function StrategyGeneration({ projectContext, selectedAgentIds, onComplete }: StrategyGenerationProps) {
  ...
  useEffect(() => {
    if (!generationStarted.current && selectedAgentIds.length > 0) {
      runGeneration()
    }
  }, [selectedAgentIds])
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-12 font-sans tracking-tight">
      <div className="w-full max-w-2xl space-y-12 text-center">
        <div className="space-y-4">
           ...
           <p className={`text-sm ${error ? 'text-red-500 font-medium px-4' : 'text-muted-foreground'}`}>{error || status}</p>
        </div>
        ...
        {error && (
          <motion.button
            ...
            onClick={handleRetry}
            disabled={isRetrying}
            className="inline-flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground rounded-full text-sm font-medium hover:opacity-90 transition-opacity shadow-lg shadow-primary/10"
          >
